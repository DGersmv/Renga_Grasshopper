...
**Решение**: 
- **Использовать методы API Renga для точного определения направления дуги:**
  - `IArc2D.IsClockwise()` - возвращает направление дуги (по часовой стрелке или против)
  - `IArc2D.GetBeginGlobalAngle()` - начальный угол в глобальной системе координат
  - `IArc2D.GetEndGlobalAngle()` - конечный угол в глобальной системе координат
- **НЕ использовать `Math.Atan2` для расчета углов** - это может дать неправильное направление
- **НЕ выбирать кратчайшую дугу** - использовать правильное направление из `IsClockwise()`
- Если `IsClockwise() == false` (против часовой стрелки) и `endAngle < startAngle`, добавлять `2π` к `endAngle`
- Если `IsClockwise() == true` (по часовой стрелке) и `endAngle > startAngle`, вычитать `2π` из `endAngle`
- Строго соблюдать линию привязки стен - baseline должен точно соответствовать положению стены
- Использовать `GetBeginGlobalAngle()` и `GetEndGlobalAngle()` вместо расчета через `Math.Atan2` для точности
